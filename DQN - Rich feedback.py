...
def dqn(n_episodes=200, max_t=1000, eps_start=0.01, eps_end=0.01, wt_start = 1, wt_end = 0.1, 
    ...
    f_agent = EAgent.EstAgent(state_size=8, action_size=4, seed=0)
    ...
    for j in range(model):
        ...
        for i in range(max_t):
            sof -= 1
            action = np.random.choice([0,1,2,3], p = oracle.act(state, 0) )
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(max_t):
            ...
            cnt+=1
            prob  = fb_agent.act(state,0) + agent.act(state, eps) + t_agent.act(state, 0) #* weight_by_frame(cnt)
            ...
            next_state, reward, done, _ = env.step(action)
            est = float(oracle.estimate(next_state) - oracle.estimate(state)) + reward
