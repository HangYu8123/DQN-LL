...
def dqn(n_episodes=200, max_t=1000, eps_start=0.01, eps_end=0.01, wt_start = 1, wt_end = 0.1, 
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(max_t):
            ...
            cnt+=1
            prob  = 0.3*fb_agent.act(state,0) + agent.act(state, eps) #* weight_by_frame(cnt)
            ...
            if cnt % 5 == 0:
                if action == np.random.choice([0,1,2,3], p = oracle.act(state, 0)):
                    feedback += 1
                else:
                    feedback  -= 1
            #evaluation-------------------------------
